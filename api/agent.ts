...
export default async function handler(request: Request) {
  ...
  const system = [{
    ...
    content:
      "Você é um agente comercial/financeiro do Grupo DV. Responda em PT-BR de forma objetiva. " +
      "Quando precisar de dados, chame as ferramentas do Varejo Fácil. Cite período e empresa na resposta."
  }];
  ...
  const toolCalls = first.output?.filter((p: any) => p.type === "tool_call") || [];
